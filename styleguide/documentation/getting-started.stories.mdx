import { Meta } from '@storybook/addon-docs/blocks';

<Meta title="Welcome/Getting started" />

# Getting started

👋🏼 Hi there! Let's get you started.

## Using with Rails

The design system package is made up of five parts:

- Haml partials. Designed to be used in Rails apps.
- Locale files. A set of bundled locale files required by Haml templates.
- SCSS files containing CSS styles for the design system
- JavaScript modules. Required for the navigation, header, tables and targeted-content components.
- Fonts. Our brand font and an icon font.

You can install the package from npm:

```sh
npm install @citizensadvice/design-system
```

### Haml

To use the provided Haml partials you will need to add the following to your Rails `ApplicationController`:

```ruby
prepend_view_path(Rails.root.join("node_modules"))
```

Once you've done this you can reference them from your app as follows:

```haml
= render partial: "@citizensadvice/design-system/haml/breadcrumb", locals: { breadcrumbs: { "breadcrumb_links" => [{ "url" => "/benefits/", "title" => "Benefits" },{ "url" => "/benefits/", "title" => "A really quite a long page title that is almost too long" }, { "url" => "/benefits/benefits-introduction/", "title" => "Benefits - introduction"}, { "url" => "/benefits/benefits-introduction/what-benefits-can-i-get/", "title" => "Benefit calculators: what benefits can you get" }] } }
```

Most components include sample code you can copy and paste. Look for the "Show code" button next to each story.

### Locale files

We export the locales folder which includes both `en.yml` and `cy.yml`. The files will be located in `node_modules/@citizensadvice/design-system/locales`.

You can include them in your rails app by adding a `config/initializers/locales.rb` file to your Rails app:

```ruby
# frozen_string_literal: true

I18n.available_locales = [:en, :cy]

# Add design-system locales to the load path
I18n.load_path += Dir[
  Rails.root.join("node_modules/@citizensadvice/design-system/locales/*.yml"),
]
```

See the [Rails i18n guide](https://guides.rubyonrails.org/i18n.html#configure-the-i18n-module) for more details.

### CSS

The recommended way of using the CSS from the design system is through Sass.

If you are using Webpacker for your assets you can load the design-system SCSS files by adding the following to your stylesheet:

```scss
@import '~@citizensadvice/design-system/scss/lib.scss';
```

The distributed package includes compiled CSS in `lib/lib.css` if you are not using Sass.

### Fonts

The design system uses a custom font to distribute it's icons. These are distributed with the npm packages and can be found under the `fonts` directory.

If you are using SCSS files through webpack then you should use the `resolve-url-loader` in your plugin chain. It should be included before the scss loader.

If you are using Webpacker with Rails then place the following in `config/webpack/environment.js`:

```js
// resolve-url-loader must be used before sass-loader
environment.loaders.get('sass').use.splice(-1, 0, {
  loader: 'resolve-url-loader',
});
```

### Javascript components

If using Webpacker, JavaScript components can be included as part of your application bundle.

Components which require JavaScript include the expected usage in their documentation. Typical usage is as follows:

```js
import header from '@citizensadvice/design-system/lib/header.js';
import greedyNav from '@citizensadvice/design-system/lib/greedy-nav/index';
import targetedContent from '@citizensadvice/design-system/lib/targeted-content.js';
import tables from '@citizensadvice/design-system/lib/tables.js';
import initOISCWarnings from '@citizensadvice/design-system/lib/oisc-warnings';

window.addEventListener('load', () => {
  header();
  greedyNav.init();
  targetedContent();
  tables();
  initOISCWarnings();
});
```

### Polyfills

Some components require polyfills for older browsers. Components which require polyfills note this in their documentation. The full list is:

- Targeted content: [Element.prototype.closest polyfill](https://www.npmjs.com/package/element-closest)
- OISC Warning: [IntersectionObserver polyfill](https://github.com/w3c/IntersectionObserver/tree/master/polyfill)

If you are not using either of these components then you may not need any polyfills.
