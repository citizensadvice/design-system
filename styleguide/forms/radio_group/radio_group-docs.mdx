...
<Preview>
  <Story id="forms-radio-group--optional-field" />
</Preview>
...
### Radio button ids (View component only)

The `id` attribute for each input will be automatically generated for you by the view component. They will take the form of:
`[name]-[index]`, where `[index]` is the position of the radio button in the array passed into `c.inputs` above. eg `my-button-0`, `my-button-1`, etc.
