...
import templateFullWidth from './_full_width.html.haml';
...
When using breadcrumbs at the top of a page, specifcy `full_width: true` inside the `breadcrumbs` locals hash. See sample pages
for an example of usage.

This variant will responsively scale it's padding so that breadcrumbs align with site content.

<Preview>
  <Story
    name="Breadcrumbs (full width)"
    parameters={{ docs: { source: { code: templateFullWidth.raw } } }}
  >
    {(_, options) => translate(templateFullWidth, options)}
  </Story>
</Preview>
...
| `full_width`       | Optional, set to true for full width breadcrumbs       |
