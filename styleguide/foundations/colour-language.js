...
function getColours() {
 const keys = Object.keys(sassColours).filter((item) => item.includes('language'));
...
export function colourLanguageTable(mappings) {
  ...
  mappings.forEach(mapping => {
    ...
    result += `<td><code>${prepareForMarkdown(`$cads-language__${colourKey}`)}</code></td>`;
    result += `<td><code>${prepareForMarkdown(colourFor(colourKey))}</code></td>`;
    result += '</tr>';
