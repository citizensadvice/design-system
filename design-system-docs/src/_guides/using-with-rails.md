...
## Using the Design System engine

The second part of the Design System is the `citizens_advice_components` gem. The Rails components can be installed by adding the following to your `Gemfile`:
...
To our `ApplicationController` we need to add the design system helpers, this makes shared view helper available to our application.
...
<html class="no-js" lang="<%%= I18n.locale %>">
  <head>
    ...
    <%%# Adds custom design system meta properties including font preloading %>
    <%%= cads_meta_head %>
...
The Design System components are built on top of [ViewComponent](https://viewcomponent.org/). You can use this library to build your own application components too. In fact let's use it to customise the [header component](/components/header) and [navigation component](/components/navigation) to suit our application.
