...
function compareSizes(oldStats, newStats, threshold) {
  ...
  newStats.forEach((newItem) => {
    ...
    if (oldSize) {
      ...
      if (diffBytes === 0) {
        log(chalk.green.dim(`${filename} has not changed (${newSize}b)`));
      } else if (diffBytes > 0) {
...
function displaySizes(sizes) {
  sizes.forEach((item) => {
    log(chalk.blue.dim(`${item.filename} is ${Math.round(item.size / 1024)}k`));
  });
}
...
function compareAndStore(sizes, threshold, statsFile, currentVersion) {
  ...
  if (!silent && filesOverThreshold) {
    log(chalk.red(`These files have changed size over ${threshold * 100}%:`));
